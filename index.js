    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            webApp.post("/wakeup", (req, res) => {
                ...
                let reason = req.headers && req.headers.wakesrc !== undefined ? req.headers.wakesrc : "unknown";
            ...
            webApp.post("/createRS", async (req, res) => {
                ...
                if (devPK && adpTkn) {
                    res.send({ rs: createRS(devPK, adpTkn) });
                } else {
            ...
            webApp.post("/cookieData", (req, res) => {
                ...
                if (saveFile) {
                    ...
                    sendCookiesToEndpoint(configData.settings.appCallbackUrl ? String(configData.settings.appCallbackUrl).replace("/receiveData?", "/cookie?") : null, sessionData.cookieData).then((sendResp) => {
                        if (sendResp) {
                            res.send("done");
                        } else {
                            res.send("failed");
            ...
            webApp.get("/clearAuth", urlencodedParser, async (req, res) => {
                logger.verbose("got request for to reload authentication");
                await clearAuth();
                res.send({
                ...
                alexaCookie.refreshAlexaCookie(
                    ...
                    async (err, result) => {
                        if (result && Object.keys(result).length >= 2) {
                            isCookieValid(result).then(async (valid) => {
                                if (valid) {
                                    await sendCookiesToEndpoint(configData.settings.appCallbackUrl ? String(configData.settings.appCallbackUrl).replace("/receiveData?", "/cookie?") : null, result);
...
async function clearAuth() {
    logger.verbose("got request for to reload authentication from HE");
    ...
    await clearSession(clearUrl, configData.settings.useHeroku);
...
async function startWebServer(checkForCookie = false) {
...
function alexaLogin(username, password, alexaOptions, callback) {
    ...
    getRemoteCookie(alexaOptions).then((remoteCookies) => {
        ...
        if (remoteCookies !== undefined && Object.keys(remoteCookies).length > 0 && remoteCookies.cookieData && remoteCookies.cookieData.localCookie && remoteCookies.cookieData.csrf) {
            ...
            alexaCookie.generateAlexaCookie(username, password, alexaOptions, async (err, result) => {
                ...
                if (err && (err.message.startsWith("Login unsuccessful") || err.message.startsWith("Amazon-Login-Error:") || err.message.startsWith(" You can try to get the cookie manually by opening"))) {
                    ...
                    if (result && result.csrf && (result.cookie || result.localCookie)) {
                        ...
                        await sendCookiesToEndpoint(alexaOptions.callbackEndpoint, result);
                        ...
                        if (alexaOptions.callbackEndpoint) {
                            await clearSession(alexaOptions.callbackEndpoint);
                        }
                    }
                }
            });
...
function getRemoteCookie(alexaOptions) {
    ...
    return new Promise((resolve) => {
        ...
        let config = {};
        if (alexaOptions.callbackEndpoint) {
            getCookiesFromEndpoint(alexaOptions.callbackEndpoint).then((data) => {
                if (data) {
