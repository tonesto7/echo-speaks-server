...
function loadConfig() {
    ...
    configFile.set("settings.hubPlatform", "Hubitat");
...
const getLocalHost = (noPort = false) => {
    return `${configData.settings.ipAddress}${noPort || configData.settings.useHeroku ? "" : `:${configData.settings.serverPort}`}`;
};
    ...
    return new Promise((resolve, reject) => {
        try {
            webApp.listen(configData.settings.serverPort, () => {
                logger.info(`** Echo Speaks Config Service (v${appVer}) is Running at (IP: ${configData.settings.ipAddress} | Port: ${configData.settings.serverPort}) | ProcessId: ${process.pid} **`);
            ...
            webApp.get("/cookie-success", (req, res) => {
                ...
                res.send(loginSuccessHtml(host));
            });
            resolve(true);
        } catch (ex) {
            reject(ex);
...
function startWebServer(checkForCookie = false) {
    ...
    const alexaOptions = {
        ...
        setupProxy: true,
        proxyOwnIp: configData.settings.ipAddress || getIPAddress(),
...
function sendServerDataToHub() {
    ...
    return new Promise((resolve) => {
        if (url) {
            ...
            // console.log("Sending Server Data to Hub: ", options);
            reqPromise(options)
                .then((resp) => {
                    // console.log("resp:", resp);
                    if (resp) {
                        logger.info(`** ServerVersion Sent to ${configData.settings.hubPlatform} Endpoint Successfully! **`);
                        resolve(true);
                    } else {
                        resolve(false);
                    }
...
function sendCookiesToEndpoint(url, cookieData) {
    return new Promise((resolve) => {
        if (url && cookieData && Object.keys(cookieData).length >= 2) {
            ...
            // console.log("sendCookiesToEndpoint: ", options);
            reqPromise(options)
                .then((resp) => {
                    // console.log('resp:', resp);
                    if (resp) {
...
function getIPAddress() {
    ...
    Object.keys(interfaces).forEach((dev) => {
        interfaces[dev].filter((details) => {
            // console.log(details);
            const cidr = details.cidr.split("/");
            const cidr24 = cidr.indexOf("24");
            if (details.family === "IPv4" && details.internal === false && cidr24 !== -1) {
...
const loginSuccessHtml = (host) => {
    let html = "";
    let redirUrl = configData.settings.useHeroku ? "https://" + host + "/config" : "http://" + host + "/config";
    html += "<!DOCTYPE html>";
    html += "<html>";
    html += "   <head>";
